...
export function projectIntakePage() {
  ...
  <section class="section intake-section"><div class="container-wide intake-layout">
    ...
    <form class="project-intake-form" data-project-intake-form enctype="multipart/form-data" novalidate>
      ...
      <fieldset><legend><span>01</span> Choose the pages for this build</legend><p>Select up to seven individual pages. A custom page can be named below.</p><div class="page-choice-grid">${pageChoice('home', 'Home', 'Main offer, proof, and next action')}${pageChoice('about', 'About', 'Story, approach, and trust')}${pageChoice('services', 'Services', 'Clear service or offer structure')}${pageChoice('shop', 'Shop / products', 'Products, collections, or packages')}${pageChoice('portfolio', 'Portfolio / gallery', 'Projects, work, or visual proof')}${pageChoice('faq', 'FAQ', 'Objections and useful answers')}${pageChoice('contact', 'Contact / quote', 'Enquiry or booking path')}${pageChoice('custom', 'Another page', 'Name it in the project notes')}</div></fieldset>
