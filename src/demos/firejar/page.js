...
export function page() {
  const cards = PRODUCTS.map((product) => `<article class="product-card" data-product data-heat="${product.heat}">
    <div class="product-card__media">${picture(productImages[product.id], { alt: `${product.name}, an invented Firejar spicy peanut butter product`, sizes: '(max-width: 760px) 100vw, 33vw' })}<span aria-hidden="true">${'●'.repeat(product.heat)}${'○'.repeat(3 - product.heat)}</span></div>
  ...
  return demoShell({
    ...
    body: `<section class="demo-hero demo-hero--fire demo-brand-hero"><div class="demo-brand-hero__copy"><p class="eyebrow">Fictional pantry experiment</p><h1>Sweet first. Heat with a point.</h1><p>Explore three invented flavors with original fictional product imagery—no borrowed packaging or customer claims.</p><a class="demo-jump" href="#firejar-flavors">Find your heat <span aria-hidden="true">↓</span></a></div><div class="demo-brand-hero__media">${picture(demoBrands.firejar.hero, { alt: 'Spicy peanut butter lifting from a spoon beside an invented unbranded Firejar jar', loading: 'eager', fetchpriority: 'high', sizes: '(max-width: 760px) 100vw, 56vw' })}<span class="demo-image-note">Fictional product / original image</span></div></section>
