...
export function setupOrderWizard(root = document) {
  ...
  const renderQuestionPlan = () => {
    ...
    questionPlan.innerHTML = `<dl><div><dt>Selected plan</dt><dd><strong>${escapeHtml(plan.name)}</strong> · ${escapeHtml(plan.displayPrice)} once</dd></div><div><dt>Plan focus</dt><dd>${escapeHtml(plan.summary)}</dd></div></dl><div><strong>What this project includes</strong>${planValueGroups(plan, 'order-review__value-groups')}</div>`;
  };
  const renderSummary = () => {
    ...
    summary.innerHTML = `<section class="order-review__details"><div class="order-review__plan"><div><span>Your selected project</span><strong>${escapeHtml(plan.name)}</strong></div><div class="order-review__total"><span>Due at checkout</span><strong>${escapeHtml(plan.displayPrice)}</strong></div></div><dl><div><dt>Customer</dt><dd>${value('fullName')}<small>${value('email')}</small></dd></div><div><dt>Business</dt><dd>${value('businessName')}<small>${value('businessNiche')}</small></dd></div><div><dt>Website</dt><dd>${value('websiteUrl')}</dd></div>${cinematicSummary}<div><dt>Primary goal</dt><dd>${value('mainGoal')}</dd></div><div><dt>Reference files</dt><dd>${files.length ? files.map((file) => escapeHtml(file.name)).join(', ') : 'None attached'}</dd></div><div><dt>Portfolio</dt><dd>${escapeHtml(portfolioSummary)}</dd></div></dl><p class="order-review__timing">First delivery targets 3 business days after payment and receipt of required assets.</p></section><section class="order-review__folio"><div class="order-review__included order-review__deliverables"><span class="micro-label">Commissioned project package</span><h4>Everything moving into production</h4><p class="order-review__promise">${escapeHtml(plan.summary)}</p>${planValueGroups(plan, 'order-review__value-groups')}</div><ol class="order-review__timeline" aria-label="What happens after checkout"><li><span>01</span><div><strong>Payment secured</strong><small>Stripe confirms the one-time payment.</small></div></li><li><span>02</span><div><strong>Direction confirmed</strong><small>Your brief and references become the production source.</small></div></li><li><span>03</span><div><strong>Delivered in your workspace</strong><small>Progress, choices, and final files stay together.</small></div></li></ol><a class="order-review__portfolio" href="/portfolio" target="_blank" rel="noopener noreferrer"><span><strong>Explore our working websites</strong><small>See the cinematic and standard experiences behind the work.</small></span><b aria-hidden="true">↗</b></a><p class="order-review__stripe-note">Your project request stays private. Stripe securely handles the one-time payment, taxes, and verified upgrade credit before you pay.</p></section>`;
