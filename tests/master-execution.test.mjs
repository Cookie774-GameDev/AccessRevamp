...
test('production packaging verifies every video and keeps every deploy path scrub-ready', async () => {
  ...
  const [pruneScript, optimizeScript, sourceOptimizer, sourceManifestText, ciWorkflow, workerWorkflow, netlifyConfig] = await Promise.all([
  ...
  assert.match(workerWorkflow, /Install FFmpeg for scrub-ready media/);
  assert.match(workerWorkflow, /REQUIRE_FFMPEG_OPTIMIZATION: "true"/);
