//#region src/app/lifecycle.js
function createLifecycle() {
	const cleanupCallbacks = /* @__PURE__ */ new Set();
	return {
		add(cleanup) {
...
var PARAMETER_SEGMENT = /^:([A-Za-z][A-Za-z0-9_]*)$/;
...
function createRouter({ routes, fallback, render }) {
...
//#endregion
//#region src/app/metadata.js
var routeMetadata = Object.freeze({
...
function updateDocumentMetadata(pathname, pattern = pathname) {
	...
	document.querySelector("meta[name=\"robots\"]")?.setAttribute("content", noIndexPatterns.has(pattern) ? "noindex,nofollow,noarchive" : "index,follow,max-image-preview:large");
}
//#endregion
//#region src/components/icons.js
...
var SUPPORT_EMAIL$1 = "support@accessrevamp.shop";
...
*/
var tier = (definition) => Object.freeze({
	...definition,
...
var plans = Object.freeze({
	free_snapshot: tier({
		...
		cadence: "one-time",
		features: [
			"One human-reviewed observation",
			"Evidence and confidence context",
			"One practical opportunity",
	...
	homepage_reveal: tier({
		...
		features: [
			...
			"One landing-page direction",
			"Desktop and mobile PNG exports",
			"One subtle AI-assisted motion poster ad",
			"30-day growth plan",
			"Upgrade to the $200 plan later for only $150"
		]
	}),
	complete_revamp: tier({
		...
		cadence: "one-time",
		features: [
			"Every applicable $50 Homepage Reveal deliverable",
			"Up to five agreed standard pages",
			"Responsive accessibility and performance work",
			"Five Canva-built AI-assisted motion poster ads",
			"Ten still poster variations",
			"Three business card variations",
			"Two brochure variations",
			"Before/after evidence and one retest summary",
			"Upgrade to the $250 Cinematic plan later for only $50"
		]
	}),
	cinematic_scroll: tier({
		...
		cadence: "one-time",
		features: [
			"Everything in Complete Website Revamp",
			"One motion-led narrative sequence",
			"Up to four story beats",
			"Mobile and reduced-motion fallbacks",
			"Upgrade from the $200 plan for $50"
		],
...
function planCard(plan, { featured = false, compact = false } = {}) {
	const action = plan.key === "free_snapshot" ? `<a class="button button--full" href="/contact?interest=free_snapshot" data-nav>Request ${escapeHtml(plan.name)} ${icon("arrow")}</a>` : `<button class="button button--full" type="button" data-checkout="${plan.key}">Choose ${escapeHtml(plan.name)} ${icon("arrow")}</button>`;
...
//#endregion
//#region src/data/visual-assets.js
var asset = (id, width, height) => ({
	id,
	width,
	height,
	avif: `${id}.avif`,
...
var visualAssets = {
	...
	greenlineInterface: asset("greenline-interface-01", 1440, 1e3),
	firejarInterface: asset("firejar-interface-01", 1440, 1e3),
	clearflowInterface: asset("clearflow-interface-01", 1440, 1e3),
	auditBefore: asset("audit-before-01", 1440, 1e3),
	auditAfter: asset("audit-after-01", 1440, 1e3)
};
visualAssets.greenlineHero, visualAssets.greenlineDetail, visualAssets.greenlineInterface, visualAssets.firejarHero, visualAssets.firejarGentle, visualAssets.firejarBright, visualAssets.firejarHot, visualAssets.firejarInterface, visualAssets.clearflowHero, visualAssets.clearflowDetail, visualAssets.clearflowInterface;
var escapeAttribute = (value) => String(value).replaceAll("&", "&amp;").replaceAll("\"", "&quot;").replaceAll("<", "&lt;").replaceAll(">", "&gt;");
function picture(assetRecord, { alt = "", className = "", loading = "lazy", sizes = "100vw", fetchpriority = "auto" } = {}) {
...
//#endregion
//#region src/data/showcase-media.js
var SHOWCASE_MEDIA_VERSION = "20260720-fluid-v4";
var versionedShowcase = (path) => `${path}?v=${SHOWCASE_MEDIA_VERSION}`;
var exampleWebsites = Object.freeze([
	{
		src: "/images/examples/local-brew-house.webp",
		width: 1672,
		height: 941,
		alt: "Local Brew House coffee shop homepage concept."
	},
	{
		src: "/images/examples/blueline-plumbing.webp",
		width: 1672,
		height: 941,
		alt: "BlueLine Plumbing service homepage concept."
	},
	{
		src: "/images/examples/spread-the-fire.webp",
		width: 1672,
		height: 941,
		alt: "Spread the Fire product homepage concept."
	},
	{
		src: "/images/examples/verdant-edge.webp",
		width: 1672,
		height: 941,
		alt: "Verdant Edge Lawn Care homepage concept."
	}
]);
var showcasePairs = Object.freeze([
	{
...
//#endregion
//#region src/components/order-wizard.js
var paidPlans = [
	"homepage_reveal",
	"complete_revamp",
	"cinematic_scroll"
...
function orderWizard() {
    ...
    <form class="order-wizard" data-order-wizard novalidate>
      <nav class="order-wizard__steps" aria-label="Project request progress">${[
		"Contact",
		"Plan",
		"Brief",
		"Review",
		"Payment"
	].map((label, index) => `<button type="button" data-order-step-jump="${index}" aria-current="${index === 0 ? "step" : "false"}"><span>0${index + 1}</span>${label}</button>`).join("")}</nav>
...
var processSteps = [
	[
		"01",
		"Observe",
		"Read the ordinary public experience and record possible friction."
	],
	[
		"02",
		"Verify",
		"Separate what is visible from inference, preference, and unsupported claims."
	],
	[
		"03",
		"Prioritize",
		"Agree on the visitor task and the change that matters first."
	],
	[
		"04",
		"Design",
		"Build a clear direction around the approved content and business goal."
	],
	[
		"05",
		"Implement",
		"Produce only the pages and creative work in the written scope."
	],
	[
		"06",
		"Retest",
		"Check the rebuilt experience and document what still needs attention."
	]
];
var outcomes = [
	[
		"Confusion",
		"clarity",
		"Competing messages and actions",
		"One understandable offer, proof sequence, and next step",
	...
	[
		"Browsing",
		"confident action",
		"Products or services with equal visual weight",
		"A hierarchy that helps visitors compare and decide",
	...
	[
		"Disconnected tools",
		"one growth system",
		"Website, content, and follow-up planned separately",
		"Pages and creative assets built around the same campaign goal",
		visualAssets.evidenceLayers,
		visualAssets.clearflowInterface
	]
];
var transformationPanel = ([from, to, problem, change, before, after], index) => `<article class="transformation-panel" data-reveal>
  <div class="transformation-panel__media">
    <span class="transformation-panel__before">${picture(before, {
	alt: `${from} interface state`,
	sizes: "(max-width: 760px) 100vw, 30vw"
})}</span>
    <span class="transformation-panel__after">${picture(after, {
	alt: `${to} interface direction`,
	sizes: "(max-width: 760px) 100vw, 30vw"
})}</span>
...
var faq$1 = (question, answer) => `<details><summary>${question}<span aria-hidden="true">+</span></summary><p>${answer}</p></details>`;
var exampleWebsite = (example) => `<figure class="example-website" data-reveal><img src="${example.src}" alt="${example.alt}" width="${example.width}" height="${example.height}" loading="lazy" decoding="async" draggable="false"></figure>`;
var showcasePanel = (kind, src, poster, name) => `<figure class="showcase-panel"><figcaption>${kind === "normal" ? "Normal Website" : "Cinematic Scroll Website"}</figcaption><div class="showcase-panel__media"><video data-src="${src}" poster="${poster}" muted playsinline preload="none" disablepictureinpicture controlslist="nodownload noplaybackrate nofullscreen" draggable="false" aria-label="${name} ${kind === "normal" ? "normal website" : "cinematic scroll website"} demonstration"></video><span class="showcase-panel__fallback">Media unavailable. The poster frame preserves the visual comparison.</span></div></figure>`;
var showcaseChapter = (pair, index) => `<article class="showcase-chapter" data-showcase-chapter data-progress="0"><div class="showcase-chapter__sticky" data-showcase-stage tabindex="0" aria-label="Scroll or drag to compare ${pair.name}"><div class="showcase-chapter__head"><span>0${index + 1}</span><h3>${pair.name}</h3><p>Original working demo — not a client engagement.</p></div><div class="showcase-pair">${showcasePanel("normal", pair.normal, pair.normalPoster, pair.name)}${showcasePanel("cinematic", pair.cinematic, pair.cinematicPoster, pair.name)}</div><div class="showcase-controls"><span>Scroll or drag to explore</span><label><span class="visually-hidden">${pair.name} comparison progress</span><input type="range" min="0" max="100" value="0" step="1" data-showcase-range><output data-showcase-output>0%</output></label></div></div></article>`;
function homePage() {
	const pricing = Object.values(plans).map((plan) => planCard(plan, {
		featured: plan.key === "complete_revamp",
		compact: true
	})).join("");
    ...
    <section class="section faq-section"><div class="container-wide faq-layout"><div><span class="eyebrow">Straight answers</span><h2>Clear scope before checkout.</h2></div><div class="faq-list">${faq$1("What happens if I buy the $50 plan first?", "The verified $50 purchase is credited toward a higher tier. Upgrade to the $200 plan for $150, or to the $250 plan for $200.")}${faq$1("Does the $250 plan include the $200 intake?", "Yes. It includes the Complete Website Revamp scope, the same page and style brief, plus the cinematic sequence. A verified $200 customer upgrades for $50.")}${faq$1("Can I share designs I like?", "Yes. Complete and Cinematic customers can choose page types, describe the style, upload reference images from a phone, and share website links. References guide the direction; AccessRevamp does not copy another brand’s logo, copy, imagery, or exact layout.")}${faq$1("Are the portfolio businesses real clients?", "No. Every portfolio brand is an original working demo—not a client engagement.")}</div></div></section>

    <section class="section final-cta-section"><div class="container-wide final-cta final-cta--renaissance"><div class="final-cta__image"><div class="audit-montage"><span>${picture(visualAssets.greenlineInterface, {
		alt: "Verdant Cut interface preview",
		sizes: "(max-width: 760px) 76vw, 24vw"
	})}</span><span>${picture(visualAssets.firejarInterface, {
		alt: "Ember and Jar interface preview",
		sizes: "(max-width: 760px) 70vw, 22vw"
	})}</span><span>${picture(visualAssets.clearflowInterface, {
		alt: "Clearline interface preview",
		sizes: "(max-width: 760px) 65vw, 20vw"
	})}</span></div></div><div><span class="eyebrow">Make the potential visible</span><h2>Your storefront already has potential.</h2><p>Bring one public website, one real goal, and the references that feel right for your business.</p><div class="final-cta__actions"><a class="button button--sun" href="/pricing" data-nav>Start my Homepage Reveal ${icon("arrow")}</a><a class="text-arrow" href="/contact?interest=complete_revamp" data-nav>Talk about a complete revamp ${icon("arrow")}</a></div></div></div></section>
  `, {
		home: true,
		pathname: "/",
		pageClass: "renaissance-home"
	});
}
//#endregion
...
var MEDIA_READY = 1;
var PRESENTATION_INTERVAL_MS = 1e3 / 24;
var MEDIA_SYNC_EPSILON_SECONDS = 1 / 48;
var FRAME_SETTLE_TIMEOUT_MS = 160;
var DESKTOP_SCROLL_DISTANCE_VH = 520;
var MOBILE_SCROLL_DISTANCE_VH = 560;
var MOBILE_BREAKPOINT_PX = 700;
var PRELOAD_RADIUS = 1;
var PRELOAD_ROOT_MARGIN = "220% 0px";
function setupShowcaseComparisons(root = document) {
	...
	const videoStates = /* @__PURE__ */ new WeakMap();
	...
	const seekVideoToLatest = (video, state) => {
		...
		const targetTime = clamp(state.targetTime, 0, Math.max(0, video.duration - .001));
	...
	const setActiveIndex = (nextIndex) => {
		...
		chapters.forEach((chapter, index) => {
			...
			if (index === previousIndex && !active) chapter.querySelectorAll("video").forEach((video) => stopVideo(video, videoStates.get(video)));
	...
	const presentActiveChapter = (time, force = false) => {
		...
		if (!force && !terminal && state.lastPresentationTime && time - state.lastPresentationTime < PRESENTATION_INTERVAL_MS) return;
	...
	const readScrollTargets = (time = performance.now()) => {
		...
		presentActiveChapter(time, nextActiveIndex !== activeIndex);
	...
	chapters.forEach((chapter) => {
		...
		const onPointerDown = (event) => {
			...
			try {
				state.stage.setPointerCapture(pointerId);
			} catch {}
		};
	...
	const preloadObserver = "IntersectionObserver" in globalThis ? new IntersectionObserver((entries) => entries.forEach((entry) => {
	...
	const handleVisibilityChange = () => {
		...
		if (!pageVisible) chapters.forEach((chapter) => chapter.querySelectorAll("video").forEach((video) => stopVideo(video, videoStates.get(video))));
		else scheduleScrollRead();
	};
	chapters.forEach((chapter) => {
	...
	return () => {
		...
		chapters.forEach((chapter) => {
			...
			chapterStates.get(chapter)?.stage?.style.removeProperty("--showcase-progress");
			chapter.querySelectorAll("video").forEach((video) => {
				video.pause();
				video.removeAttribute("src");
				delete video.dataset.prepared;
...
//#endregion
//#region src/services/order-wizard.js
var STORAGE_KEY$2 = "accessrevamp-order-draft-v1";
var PENDING_PLAN_KEY$1 = "accessrevamp:pending-plan";
var MAX_FILES$1 = 8;
var MAX_BYTES$1 = 8 * 1024 * 1024;
var PAID_PLANS$1 = /* @__PURE__ */ new Set([
	"homepage_reveal",
	"complete_revamp",
	"cinematic_scroll"
]);
...
function setupOrderWizard(root = document) {
	...
	const restore = () => {
		try {
			const saved = JSON.parse(localStorage.getItem(STORAGE_KEY$2) || "{}");
			Object.entries(saved.draft || {}).forEach(([name, value]) => {
				[...form.elements].filter((control) => control.name === name).forEach((control) => {
		...
		try {
			const pendingPlan = sessionStorage.getItem(PENDING_PLAN_KEY$1);
			if (PAID_PLANS$1.has(pendingPlan)) {
				const planControl = form.querySelector(`input[name="orderPlan"][value="${pendingPlan}"]`);
				if (planControl) planControl.checked = true;
				sessionStorage.removeItem(PENDING_PLAN_KEY$1);
			}
		} catch {}
		exposeRequestId();
	};
	...
	const validatePanel = () => {
		const invalid = [...panels[current].querySelectorAll("[required]")].find((control) => !control.checkValidity());
	...
	const onNext = () => {
		normalizeUrl();
		if (validatePanel()) show(current + 1);
	};
	...
	const onFiles = () => {
		...
		files = incoming.filter((file) => ALLOWED.has(file.type) && file.size <= MAX_BYTES$1).slice(0, MAX_FILES$1);
		syncFileInput();
		renderFiles();
...
//#endregion
//#region src/pages/home-interactions.js
var HERO_SETTLE_EPSILON = .08;
function setupHomeExperience(root = document) {
	...
	const paintHero = () => {
		...
		gridOffset.x += (targetGridX - gridOffset.x) * .08;
		gridOffset.y += (targetGridY - gridOffset.y) * .08;
		...
		if (Math.abs(deltaX) > HERO_SETTLE_EPSILON || Math.abs(deltaY) > HERO_SETTLE_EPSILON || Math.abs(targetGridX - gridOffset.x) > HERO_SETTLE_EPSILON || Math.abs(targetGridY - gridOffset.y) > HERO_SETTLE_EPSILON) heroFrame = requestAnimationFrame(paintHero);
	};
	const startHeroLoop = () => {
		if (!heroFrame && !reducedMotion && pageVisible && heroVisible && hero) heroFrame = requestAnimationFrame(paintHero);
	};
...
//#endregion
//#region src/data/portfolio.js
var portfolioItems = Object.freeze([
	Object.freeze({
		slug: "northline-goods",
		index: "01",
		title: "Northline Goods",
		artWord: "Made for miles.",
		category: "Outdoor storefront",
		kind: "homepage",
		...
		palette: "Sun yellow, persimmon, and deep ink",
		deliverable: "Homepage direction",
		fictionalConcept: true
	}),
	Object.freeze({
		slug: "morrow-studio",
		index: "02",
		title: "Morrow Studio",
		artWord: "Shape the quiet.",
		...
		palette: "Ultramarine, sun, and gallery white",
		deliverable: "Homepage direction",
		fictionalConcept: true
	}),
	Object.freeze({
		...
		palette: "Mint, persimmon, and parchment",
		deliverable: "Homepage direction",
		fictionalConcept: true
	}),
	Object.freeze({
		...
		palette: "Peach, persimmon, and espresso ink",
		deliverable: "Campaign direction",
		fictionalConcept: true
	}),
	Object.freeze({
		slug: "move-well",
		...
		palette: "Powder blue, ultramarine, and sun",
		deliverable: "Campaign direction",
		fictionalConcept: true
	}),
	Object.freeze({
	...
	Object.freeze({
		...
		index: "07",
		title: "Aether One",
		artWord: "Enter a clearer orbit.",
		category: "Cinematic product story",
		kind: "cinematic",
		...
		palette: "Night ink, electric blue, and lunar white",
		deliverable: "Cinematic scroll concept",
		fictionalConcept: true
	})
]);
Object.freeze([
...
var findPortfolioItem = (slug) => portfolioItems.find((item) => item.slug === slug);
//#endregion
...
function setupWorkFilters() {
	...
	const onClick = (event) => {
		...
		items.forEach((item) => {
			item.hidden = selected !== "all" && item.dataset.workKind !== selected;
		});
	};
...
//#endregion
//#region src/pages/services.js
function servicesPage() {
	return shell(`<section class="page-hero"><div class="container-wide page-hero__split"><div><span class="eyebrow">Services with a boundary</span><h1>From clearer thinking to a complete story.</h1></div><p class="lede">Start with the smallest useful intervention or choose a fuller build. The scope changes; the commitment to human review does not.</p></div></section><section class="section"><div class="container-wide service-list">${Object.values(plans).map((plan, index) => `<article class="service-row"><span>0${index + 1}</span><div><span class="kicker">${plan.label}</span><h2>${plan.name}</h2></div><p>${plan.summary}</p><a class="circle-link" href="/pricing" data-nav aria-label="See ${plan.name} pricing">${icon("arrow")}</a></article>`).join("")}</div></section><section class="section story-section"><div class="container-wide story-intro"><div><span class="eyebrow">What stays true</span><h2>Evidence before drama. Direction before decoration.</h2></div><p>No recurring AccessRevamp platform fee, no invented security claims, and no promise that a design alone guarantees revenue or compliance.</p></div></section>`, { pathname: "/services" });
}
//#endregion
...
var sample_report_default = {
	id: "fictional-northline-homepage",
	disclosure: "fictional-sample",
	title: "Northline Goods — fictional homepage reveal",
	disclaimer: "Northline Goods is invented. This demonstrates the report structure and does not claim a real audit, outcome, customer, or compliance result.",
	executiveSummary: "The sample homepage presents three equal-weight decisions before one clear offer and its supporting proof. The recommended direction reduces that competition while preserving exploration.",
	findings: [
		{
			"category": "Conversion",
			"title": "Primary action competes with navigation",
			"summary": "Three equal-weight actions arrive before supporting proof.",
			"evidence": "Static hierarchy review at desktop and mobile widths.",
			"source": "Fictional public homepage fixture",
			"referenceType": "screenshot and DOM reference",
			"severity": "high",
			"confidence": 92,
			"affectedUser": "First-time visitor",
			"affectedTask": "Choose the next useful step",
			"standardReference": "Internal hierarchy heuristic H-01",
			"recommendedFix": "Use one dominant action and demote supporting links.",
			"effort": "Small",
			"reviewer": "AccessRevamp human-review sample",
			"retestResult": "Pending sample implementation"
		},
		{
			"category": "Accessibility",
			"title": "Muted context needs contrast verification",
			"summary": "Supporting copy may be difficult to distinguish from the surface.",
			"evidence": "Sample computed-color reference requires final browser measurement.",
			"source": "Fictional style fixture",
			"referenceType": "computed style reference",
			"severity": "verify",
			"confidence": 68,
			"affectedUser": "Low-vision visitor",
			"affectedTask": "Read offer limitations",
			"standardReference": "WCAG 2.2 contrast criteria for final verification",
			"recommendedFix": "Raise foreground contrast and measure all states.",
			"effort": "Small",
			"reviewer": "AccessRevamp human-review sample",
			"retestResult": "Not yet measured"
		},
		{
			"category": "Content",
			"title": "Trust arrives after the first decision",
			"summary": "Shipping and material context appears below the initial purchase choice.",
			"evidence": "Sample content-order review.",
			"source": "Fictional content fixture",
			"referenceType": "DOM order reference",
			"severity": "medium",
			"confidence": 85,
			"affectedUser": "Risk-aware shopper",
			"affectedTask": "Assess offer confidence",
			"standardReference": "Internal content-order heuristic C-02",
			"recommendedFix": "Bring one concise reassurance beside the first action.",
			"effort": "Small",
			"reviewer": "AccessRevamp human-review sample",
			"retestResult": "Pending sample implementation"
		}
	...
	experiments: [{
		"hypothesis": "A single reassurance beside the primary action may reduce decision uncertainty.",
		"audience": "New mobile visitors",
		"change": "Add one factual shipping boundary beside the action.",
		"metric": "Primary-action completion rate",
		"guardrail": "Support contacts do not increase",
		"window": "14 days or adequate sample",
	...
	socialPlan: [
		{
			"week": 1,
			"pillar": "Clarify",
			"format": "Short carousel",
			"concept": "Explain the one customer task",
			"cta": "Read the guide",
			"measurement": "Qualified visits"
		},
		{
			"week": 2,
			"pillar": "Prove",
			"format": "Annotated still",
			"concept": "Show an evidence-to-design decision",
			"cta": "See the method",
			"measurement": "Saves and method visits"
		},
		{
			"week": 3,
			"pillar": "Teach",
			"format": "FAQ post",
			"concept": "Answer the top purchase boundary",
			"cta": "Compare options",
			"measurement": "Pricing visits"
		},
		{
			"week": 4,
			"pillar": "Invite",
			"format": "Founder note",
			"concept": "Invite one bounded next action",
			"cta": "Request a snapshot",
			"measurement": "Consent-based requests"
		}
	],
	roadmap: [
		{
			"phase": "1 — Verify",
			"work": "Measure contrast and confirm business priority.",
			"dependency": "Owner review",
			"acceptance": "Evidence record approved"
		},
		{
			"phase": "2 — Reshape",
			"work": "Implement offer, proof, and action hierarchy.",
			"dependency": "Approved copy",
			"acceptance": "Keyboard and viewport review complete"
		},
		{
			"phase": "3 — Measure",
			"work": "Run the bounded growth experiment.",
			"dependency": "Consent-aware analytics",
			"acceptance": "Result recorded without causal overclaim"
		}
	]
};
//#endregion
//#region src/report/report-model.js
var FINDING_FIELDS = [
	"category",
	"title",
	"summary",
...
function validateReport(report) {
	if (!report || report.disclosure !== "fictional-sample") throw new Error("Sample disclosure is required.");
	if (!Array.isArray(report.findings) || !report.findings.length) throw new Error("Findings are required.");
	for (const [index, finding] of report.findings.entries()) {
		for (const field of FINDING_FIELDS) if (finding[field] === void 0 || finding[field] === "") throw new Error(`Finding ${index + 1} is missing ${field}.`);
		if (!(Number.isInteger(finding.confidence) && finding.confidence >= 0 && finding.confidence <= 100)) throw new Error("Confidence must be from 0 to 100.");
	}
	return report;
}
var priority = (finding) => ({
	high: 3,
	medium: 2,
	low: 1,
	verify: 1
}[finding.severity] || 0) * finding.confidence;
//#endregion
//#region src/report/report-page.js
var findingCard = (f) => `<article class="report-finding"><span>${escapeHtml(f.category)}</span><div><h3>${escapeHtml(f.title)}</h3><p>${escapeHtml(f.summary)}</p><dl><div><dt>Evidence</dt><dd>${escapeHtml(f.evidence)}</dd></div><div><dt>Source</dt><dd>${escapeHtml(f.source)} · ${escapeHtml(f.referenceType)}</dd></div><div><dt>Affected task</dt><dd>${escapeHtml(f.affectedUser)} · ${escapeHtml(f.affectedTask)}</dd></div><div><dt>Recommendation</dt><dd>${escapeHtml(f.recommendedFix)}</dd></div><div><dt>Review</dt><dd>${escapeHtml(f.reviewer)} · retest: ${escapeHtml(f.retestResult)}</dd></div></dl></div><span class="severity">${escapeHtml(f.severity)} / ${f.confidence}%</span></article>`;
function reportPage(input) {
	const report = validateReport(input);
	const top = [...report.findings].sort((a, b) => priority(b) - priority(a)).slice(0, 5);
	return `<article class="full-report" data-report-id="${escapeHtml(report.id)}"><header><span class="eyebrow">Illustrative $50 Homepage Reveal</span><h1>${escapeHtml(report.title)}</h1><p>${escapeHtml(report.disclaimer)}</p></header><section><h2>Executive summary</h2><p>${escapeHtml(report.executiveSummary)}</p></section><section><h2>Top five priorities</h2><ol>${top.map((f) => `<li><strong>${escapeHtml(f.title)}</strong> — ${escapeHtml(f.recommendedFix)}</li>`).join("")}</ol></section><section><h2>Verified findings appendix</h2>${report.findings.map(findingCard).join("")}</section><section><h2>Monetization experiments</h2>${report.experiments.map((e) => `<article><h3>${escapeHtml(e.hypothesis)}</h3><p><strong>Audience:</strong> ${escapeHtml(e.audience)} · <strong>Metric:</strong> ${escapeHtml(e.metric)} · <strong>Guardrail:</strong> ${escapeHtml(e.guardrail)}</p><p>${escapeHtml(e.change)} Test window: ${escapeHtml(e.window)}. Human owner: ${escapeHtml(e.owner)}.</p></article>`).join("")}</section><section><h2>30-day social growth plan</h2>${report.socialPlan.map((p) => `<article><h3>Week ${p.week}: ${escapeHtml(p.pillar)}</h3><p>${escapeHtml(p.format)} · ${escapeHtml(p.concept)} · CTA: ${escapeHtml(p.cta)} · Measurement: ${escapeHtml(p.measurement)}</p></article>`).join("")}</section><section><h2>Implementation roadmap</h2><ol>${report.roadmap.map((r) => `<li><strong>${escapeHtml(r.phase)}</strong>: ${escapeHtml(r.work)} · dependency ${escapeHtml(r.dependency)} · acceptance ${escapeHtml(r.acceptance)}</li>`).join("")}</ol></section></article>`;
}
//#endregion
//#region src/pages/sample-report.js
function sampleReportPage() {
	return shell(`<section class="section report-route"><div class="container-wide">${reportPage(sample_report_default)}<p><a class="button" href="/pricing" data-nav>Get the $50 Homepage Reveal</a></p></div></section>`, { pathname: "/sample-report" });
}
//#endregion
//#region src/pages/process.js
var steps = [
	[
		"01",
		"Public-surface intake",
		"We begin with the normal public experience. No credentials, customer records, private files, or internal systems are required for an initial review."
	],
	[
		"02",
		"Passive observation",
		"We inspect ordinary page behavior without submitting forms, creating accounts, entering checkout, probing admin routes, or attempting exploitation."
	],
	[
		"03",
		"Human verification",
		"Automated signals are treated as leads. A person confirms context, softens uncertainty, and removes weak claims before delivery."
	],
	[
		"04",
		"Editorial direction",
		"We choose the story hierarchy, proof, actions, and visual language that answer the verified friction instead of applying a generic template."
	],
	[
		"05",
		"Bounded build",
		"The selected deliverable is produced against the written page count, asset, platform, motion, and revision boundaries."
	],
	[
		"06",
		"Review and handoff",
		"The delivered scope is checked again for responsive behavior, accessibility basics, and any limitations that still need to be documented."
	]
];
function processPage({ methodology = false } = {}) {
...
//#endregion
//#region src/pages/cinematic.js
function cinematicPage() {
	const markup = [
		[
			"signals",
			"01 · Scattered signals",
			"Your homepage speaks in fragments.",
			"Passive observations begin as signals, not conclusions.",
			"signal-field-01"
		],
		[
			"evidence",
			"02 · Verified evidence",
			"Verification earns the redesign.",
			"Source, observation, confidence, affected task, and human review turn a signal into usable evidence.",
			"evidence-layers-01"
		],
		[
			"hierarchy",
			"03 · Redesigned hierarchy",
			"The page learns what leads.",
			"Offer, proof, and action arrive in an intentional order with accessible fallback states.",
			"hierarchy-beacon-01"
		],
		[
			"action",
			"04 · Clear action",
			"One next move resolves the story.",
			"The visual system ends at a specific, honest conversion—not an endless effect.",
			"action-path-01"
		]
	].map(([id, label, title, copy, image]) => `<article data-cinematic-beat="${id}"><picture><source srcset="/assets/generated/${image}.avif" type="image/avif"><source srcset="/assets/generated/${image}.webp" type="image/webp"><img src="/assets/generated/${image}.webp" alt="" width="840" height="900" loading="lazy"></picture><div><span>${label}</span><h2>${title}</h2><p>${copy}</p>${id === "action" ? `<a class="button button--sun" href="/pricing" data-nav>Get the $50 Homepage Reveal ${icon("arrow")}</a>` : ""}</div></article>`).join("");
	return shell(`<section class="cinematic-intro"><div class="container-wide"><span class="eyebrow">AccessRevamp cinematic evidence story</span><h1>From scattered signals to a clear action.</h1><p>Scroll through four connected beats. Every panel remains complete and readable when motion, JavaScript, or media is unavailable.</p><a class="button button--sun" href="#cinematic-stage">Enter the evidence story ${icon("arrow")}</a></div></section><section class="cinematic-stage" id="cinematic-stage" data-cinematic-stage><div class="cinematic-sticky"><div class="cinematic-beats">${markup}</div><div class="cinematic-progress" aria-hidden="true"><span data-cinematic-progress></span></div></div></section><section class="section cinematic-scope"><div class="container-wide page-hero__split"><div><span class="eyebrow">Bounded production</span><h2>Premium movement, with practical limits.</h2></div><div><p class="lede">One cohesive sequence, up to four story beats, one revision round, plus mobile, static, reduced-motion, and failed-media fallbacks.</p><a class="button" href="/pricing" data-nav>See the $250 plan ${icon("arrow")}</a></div></div></section>`, {
		pathname: "/cinematic-scroll",
		pageClass: "cinematic-page"
	});
}
//#endregion
//#region src/pages/contact.js
var offers = [
...
function readContactInterest(search = globalThis.location?.search || "") {
...
function contactPage() {
	...
	return shell(`
    <section class="page-hero"><div class="container-wide page-hero__split"><div><span class="eyebrow">Contact AccessRevamp</span><h1>What should your website make easier?</h1></div><div class="page-hero__aside"><p class="lede">Share a public URL and the outcome you care about. Never send passwords, access tokens, private customer data, or payment details.</p>${siteConfig.contactEmail ? `<p>Prefer email? <a href="mailto:${escapeHtml(siteConfig.contactEmail)}">${escapeHtml(siteConfig.contactEmail)}</a></p>` : ""}</div></div></section>
    <section class="section"><div class="container-wide contact-layout">
      <aside class="contact-aside"><span class="eyebrow">What happens next</span><h2>One public site. One real goal.</h2><ol><li>Your request is validated and recorded.</li><li>A person reviews the public page and stated goal.</li><li>You receive the next-step boundary by email.</li></ol><div class="privacy-note">Requests are rate-limited and stored through a server-only Supabase function after deployment configuration.</div></aside>
      <form class="contact-form" data-contact-form novalidate>
        <label>Selected offer<select name="interest"><option value="">Choose an offer</option>${offers.map(([value, label]) => `<option value="${value}"${selected === value ? " selected" : ""}>${label}</option>`).join("")}</select></label>
...
//#endregion
//#region src/pages/account-projects.js
function accountProjectsPage({ pathname = "/account/projects" } = {}) {
...
//#endregion
//#region src/pages/dashboard.js
function dashboardPage() {
	return accountProjectsPage({ pathname: "/dashboard" });
}
//#endregion
//#region src/data/policies.js
var refundPolicy = Object.freeze({
	title: "Refund policy",
	summary: "You may request a full refund before final digital delivery when the service has not been delivered.",
	delivery: "Final delivery occurs when the agreed digital files, report, or published scope is made available to you.",
	timing: "The Cinematic Scroll Site target is three business days after payment and complete intake, subject to the written scope and timely feedback.",
	rights: "Nothing in this policy limits consumer rights that cannot legally be excluded."
});
//#endregion
//#region src/pages/legal.js
var SUPPORT_EMAIL = "support@accessrevamp.shop";
var LEGAL_LAST_UPDATED = "July 22, 2026";
var routeFor = Object.freeze({
	legal: "/legal",
	privacy: "/privacy",
	policy: "/policy",
	terms: "/terms",
		...
		sections: [
			{
				...
				paragraphs: ["This policy applies to accessrevamp.com, AccessRevamp account and customer-workspace pages, project intake, support communications, checkout-related records, and files delivered through the service.", "AccessRevamp is the controller of personal information described in this policy unless a project-specific agreement states otherwise. Third-party websites and services have their own privacy notices."]
			},
			{
				id: "collection",
			...
			},
			{
				...
				paragraphs: ["AccessRevamp uses essential browser storage and session technologies for navigation, account security, authentication, checkout return state, and customer-workspace functions. These technologies are necessary to provide the requested service.", "AccessRevamp does not sell personal information and does not currently use customer project data for cross-context behavioral advertising. If materially different analytics or advertising practices are introduced, this policy and any required consent controls will be updated first."]
			},
			{
				id: "retention",
				title: "7. Retention",
			...
			},
			{
				id: "security",
				title: "8. Security",
				paragraphs: ["AccessRevamp uses access controls, email confirmation, password-plus-email sign-in checks, private storage, expiring download links, encrypted transport, rate limits, and restricted operator access. No system can guarantee absolute security, so customers should use a unique password, protect verification messages, and report suspected account misuse promptly."]
			},
			{
				id: "rights",
				title: "9. Your privacy rights",
				paragraphs: ["Depending on your location, you may have rights to request access, correction, deletion, restriction, portability, or objection; withdraw consent; appeal a decision; or complain to a regulator. Where applicable, California residents may also request details about collection and disclosure and may exercise rights without unlawful discrimination. AccessRevamp does not sell personal information or share it for cross-context behavioral advertising.", `Send requests to <a href="${mailto}">${SUPPORT_EMAIL}</a>. AccessRevamp may verify identity and may retain information when required by law, needed to complete a transaction, protect security, or establish legal claims.`]
			},
			{
				id: "international",
				title: "10. International processing",
				paragraphs: ["Service providers may process information in countries other than the one where you live. Where required, AccessRevamp relies on contractual, organizational, or other lawful transfer safeguards."]
			},
			{
				id: "children",
				title: "11. Children",
				paragraphs: ["AccessRevamp is intended for businesses and people able to enter a binding service agreement. It is not directed to children under 13, and AccessRevamp does not knowingly collect personal information from children under 13. A parent or guardian who believes a child supplied information should contact support."]
			},
			{
				id: "changes",
				title: "12. Changes and contact",
				paragraphs: [`Material changes will be posted on this page with a new effective date. Privacy questions and requests may be sent to <a href="${mailto}">${SUPPORT_EMAIL}</a>. Do not email passwords, one-time codes, full card numbers, or private access tokens.`]
			}
		]
	},
	policy: {
		...
		sections: [
			...
			},
			{
				...
				paragraphs: ["The service scope is the plan purchased together with the accepted project brief, written clarifications, and documented approvals. Work outside that scope may require a separate quote, revised timeline, or new order.", "A request is not accepted merely because it was mentioned in a message. Material additions are accepted only after AccessRevamp confirms the scope, cost, and timing in writing."]
			},
			{
				id: "inputs",
			...
			},
			{
				...
				paragraphs: ["Revision allowances are stated in the selected plan or written scope. A revision changes an existing approved direction; a new direction, additional page, new feature, or materially different sequence may be treated as added scope.", "An approval may be recorded through the customer workspace, approval link, or clear written confirmation. AccessRevamp may rely on the latest approval when continuing production."]
			},
			{
				id: "timing",
				title: "5. Timing and delays",
				paragraphs: ["Delivery estimates are good-faith estimates, not guaranteed deadlines, unless a separate signed agreement expressly states otherwise. Timelines may move when content, access, approvals, third-party services, customer responses, or events outside reasonable control cause delay."]
			},
			{
				id: "delivery",
			...
			},
			{
				...
				paragraphs: ["AccessRevamp tests work against the agreed scope and aims for responsive, accessible, and secure implementation. Automated audits and observations are not certifications, legal opinions, penetration tests, or guarantees that every third-party platform, browser, assistive technology, integration, or future update will behave identically.", "Third-party services may change pricing, limits, APIs, policies, availability, or output. AccessRevamp is not responsible for a third party’s independent service, but will use reasonable care when integrating the agreed tools."]
			},
			{
				id: "conduct",
				title: "8. Acceptable use and suspension",
				paragraphs: ["The service may not be used for unlawful content, infringement, fraud, malware, harassment, impersonation, unauthorized security testing, or abuse of another person’s privacy. AccessRevamp may pause work or restrict access when reasonably necessary to protect customers, the service, third parties, or legal compliance."]
			},
			{
				id: "concerns",
				title: "9. Service concerns",
				paragraphs: [`Report a concern promptly to <a href="${mailto}">${SUPPORT_EMAIL}</a> with the project name, affected page or file, expected result, and screenshots when helpful. AccessRevamp will review the documented scope, delivery record, and applicable refund terms before proposing a resolution.`]
			}
		]
	},
	terms: {
		...
		sections: [
			{
				...
				paragraphs: ["You must be able to enter a binding agreement and must provide accurate information. If you act for a company or another person, you represent that you have authority to bind them. Project-specific written terms control over these public terms if they directly conflict."]
			},
			{
				id: "accounts",
				title: "2. Accounts and security",
				paragraphs: ["You are responsible for your password, verification messages, device access, and activity under your account. Notify AccessRevamp promptly if you suspect unauthorized access. AccessRevamp may require renewed verification, revoke sessions, or restrict access to protect the account."]
			},
			{
				id: "orders",
				title: "3. Orders, prices, and payment",
				paragraphs: ["Prices and included work are shown at checkout or in a written quote. Taxes, third-party fees, licensed assets, advertising spend, domain purchases, premium software, and services outside the stated scope are not included unless expressly listed. Payment is processed by an independent payment processor under its terms.", "An order is accepted when payment is verified and AccessRevamp confirms the applicable entitlement or project record. AccessRevamp may cancel and refund an order affected by a pricing error, fraud concern, legal restriction, or inability to provide the service."]
			},
			{
			...
			},
			{
				...
				paragraphs: ["AccessRevamp will perform the agreed service with reasonable care and skill. Except for express written commitments and rights that cannot be excluded, the website, previews, recommendations, and deliverables are provided without additional warranties, including implied warranties of merchantability, fitness for a particular purpose, uninterrupted availability, or guaranteed business results.", "Marketing, conversion, search ranking, accessibility, security, legal compliance, revenue, or platform approval outcomes are not guaranteed. You remain responsible for business decisions, published claims, legal notices, regulated content, and final acceptance testing for your use case."]
			},
			{
				id: "liability",
				title: "6. Limitation of liability",
				paragraphs: ["To the fullest extent permitted by law, AccessRevamp is not liable for indirect, incidental, special, consequential, exemplary, or punitive damages; lost profits, revenue, data, goodwill, or business opportunity; or losses caused by third-party services, customer content, unauthorized account access, or use outside the agreed scope.", "To the fullest extent permitted by law, AccessRevamp’s aggregate liability arising from an affected service is limited to the amount paid to AccessRevamp for that service during the twelve months before the event giving rise to the claim. These limits do not apply where liability cannot legally be limited or excluded."]
			},
			{
				id: "indemnity",
				title: "7. Responsibility for supplied materials and misuse",
				paragraphs: ["To the extent permitted by law, you are responsible for claims arising from content, instructions, data, access, or materials you supply; your unlawful or unauthorized use of the service; or your breach of another party’s rights, except to the extent caused by AccessRevamp’s own breach or misconduct."]
			},
			{
				id: "termination",
				title: "8. Suspension and termination",
				paragraphs: ["AccessRevamp may suspend or terminate access for material breach, fraud, nonpayment, unlawful activity, security risk, abuse, or conduct that threatens the service or others. Where practical, AccessRevamp will provide notice and an opportunity to cure. Terms that by their nature should continue—including payment, ownership, disclaimers, liability limits, and dispute provisions—survive termination."]
			},
			{
				id: "law",
				title: "9. Disputes, governing rules, and general terms",
				paragraphs: [`Contact <a href="${mailto}">${SUPPORT_EMAIL}</a> first so the parties can try to resolve a dispute informally. These terms are governed by the laws applicable to AccessRevamp’s principal place of business, without overriding mandatory consumer protections that apply to you. A project-specific agreement may identify a more specific governing law or forum.`, "If one provision is unenforceable, the remaining provisions continue. Failure to enforce a provision is not a waiver. You may not transfer an order or account without consent; AccessRevamp may transfer the agreement as part of a business reorganization or sale. Electronic notices and approvals may satisfy writing requirements where permitted."]
			},
			{
				id: "updates",
				title: "10. Changes",
				paragraphs: ["Updated terms apply prospectively from the posted effective date unless law requires otherwise. Material changes affecting an active paid project will not reduce the confirmed scope without agreement."]
			}
		]
	},
	accessibility: {
		...
		sections: [
			{
				id: "commitment",
				title: "1. Commitment",
				paragraphs: ["AccessRevamp aims to provide a perceivable, operable, understandable, and robust public website and customer workspace. The service uses semantic structure, labels, visible focus, status announcements, contrast checks, responsive layouts, and reduced-motion handling."]
			},
			{
				id: "testing",
				title: "2. Testing and limitations",
				paragraphs: ["Automated checks are combined with manual review. No automated score or single test can prove complete conformance across every browser, device, assistive technology, customer-supplied asset, or third-party component. Confirmed limitations are prioritized according to impact and the available remedy."]
			},
			{
				id: "feedback",
				title: "3. Accessibility support",
				paragraphs: [`Email <a href="${mailto}">${SUPPORT_EMAIL}</a> with the page or task, what you expected, what happened, and your browser or assistive technology if you are comfortable sharing it. Do not include passwords or verification codes.`]
			},
			{
				id: "response",
				title: "4. Response",
				paragraphs: ["AccessRevamp will acknowledge actionable accessibility reports, investigate reproducible barriers, and provide a resolution, workaround, or documented limitation when reasonably possible."]
			}
		]
	},
	outreach: {
		...
		sections: [
			{
				id: "source",
				title: "1. Sources and relevance",
				bullets: [
					"Use only intentionally published business contact details and record the public source.",
					"Base each message on a human-verified observation from the reviewed public page.",
					"Do not infer sensitive traits or use purchased personal-contact lists."
				]
			},
			{
				id: "message",
				title: "2. Message requirements",
				bullets: [
					"Identify AccessRevamp accurately and use a working reply address.",
					"Keep claims accurate, specific, and proportionate to the evidence.",
					"Do not use fake reply subjects, false urgency, invented relationships, unverified security claims, or legal threats. automated spam-classification manipulation is prohibited.",
					"Include a clear way to opt out and honor it."
				]
			},
			{
				id: "suppression",
				title: "3. Opt-outs and complaints",
				paragraphs: [`Opt-out and outreach complaints may be sent to <a href="${mailto}">${SUPPORT_EMAIL}</a>. Suppressed recipients remain excluded from future outreach unless they later make a clear new request.`]
			}
		]
	},
	support: {
		...
		sections: [
			{
				...
				paragraphs: [`Email <a href="${mailto}">${SUPPORT_EMAIL}</a>. Support is currently provided by email; there is no public phone-support line or emergency response service.`, "For the fastest match, write from the confirmed email used for the AccessRevamp account, order, or project."]
			},
			{
				id: "include",
			...
			},
			{
				...
				paragraphs: ["After a verified sign-in, the dashboard can show project status, progress updates, brief and references, designs for review, approved videos, reports, website packages, and download links. Links may expire and can be refreshed by reloading the workspace.", "When a video or file is published, it becomes available in the private dashboard. AccessRevamp may also send a customer email directing the customer to sign in and view or download it."]
			},
			{
				id: "security",
				title: "5. Security and abuse reports",
				paragraphs: [`Send suspected account compromise, exposed private links, impersonation, or security concerns to <a href="${mailto}">${SUPPORT_EMAIL}</a> with “Security” in the subject. Do not perform active testing against systems you do not own or lack written authorization to test.`]
			},
			{
				id: "expectations",
				title: "6. Response expectations",
				paragraphs: ["AccessRevamp aims to acknowledge messages within two business days. Resolution time depends on identity verification, project complexity, provider availability, and whether the request involves billing, privacy, accessibility, or a security investigation. Response targets are goals, not guaranteed service-level commitments."]
			}
		]
	},
	legal: {
		...
		sections: [
			{
				...
				paragraphs: ["Review the information AccessRevamp collects, how it is used, service-provider categories, retention, security controls, and available privacy rights."],
				link: ["/privacy", "Read the Privacy Policy"]
			},
			{
				id: "policy",
				title: "Customer service policy",
				paragraphs: ["Review project scope, communication, revisions, approvals, timing, private delivery, support, and acceptable-use rules."],
				link: ["/policy", "Read the Customer Policy"]
			},
			{
				id: "terms",
				title: "Terms of service",
				paragraphs: ["Review the contract rules for accounts, orders, content rights, service standards, disclaimers, liability, and disputes."],
				link: ["/terms", "Read the Terms"]
			},
			{
				id: "refunds",
				title: "Refund policy",
				paragraphs: ["Review cancellation and refund handling before final digital delivery and the rights that remain under applicable law."],
				link: ["/refunds", "Read the Refund Policy"]
			},
			{
				id: "accessibility",
				title: "Accessibility statement",
				paragraphs: ["Review the accessibility commitment, testing approach, limitations, and barrier-reporting channel."],
				link: ["/accessibility", "Read the Accessibility Statement"]
			},
			{
				id: "support",
				title: "Customer support",
				paragraphs: [`Get help with accounts, orders, projects, delivery files, privacy, accessibility, or security through <a href="${mailto}">${SUPPORT_EMAIL}</a>.`],
				link: ["/support", "Open Customer Support"]
			}
		]
	}
};
function refundPage() {
	return {
		...
		sections: [
			{
				id: "before-delivery",
				title: "1. Before final delivery",
				paragraphs: [refundPolicy.delivery]
			},
			{
				id: "cinematic-timing",
				title: "2. Cinematic timing and reserved production",
				paragraphs: [refundPolicy.timing]
			},
			{
				id: "rights",
				title: "3. Rights preserved",
				paragraphs: [refundPolicy.rights]
			},
			{
				id: "request",
				title: "4. Requesting cancellation or a refund",
				paragraphs: [`Email <a href="${mailto}">${SUPPORT_EMAIL}</a> from the account or order email. Include the plan, purchase date, order reference if available, and the reason for the request. Do not include full card details.`]
			},
			{
				id: "review",
				title: "5. Review and outcome",
				paragraphs: ["AccessRevamp will review the verified payment record, project status, delivered files, approved scope, completed work, non-recoverable third-party costs, and applicable law before confirming the outcome. Refunds, when approved, are returned through the original payment method where reasonably possible. Processing time may depend on the payment provider and bank."]
			}
		]
	};
}
...
function legalPage(kind = "legal", pathname = routeFor[kind] || "/legal") {
	const page = kind === "refunds" ? refundPage() : pages[kind] || pages.legal;
	const supportLink = `${mailto}?subject=${encodeURIComponent(`AccessRevamp support — ${page.kicker}`)}`;
    ...
    <section class="section policy-contact">
      <div class="container-wide policy-contact__card"><div><span class="eyebrow">Need a human response?</span><h2>Contact AccessRevamp customer support.</h2><p>Use the confirmed account or order email when possible. Never send passwords, one-time codes, full card numbers, private keys, or access tokens.</p></div><div><a class="button" href="${supportLink}">${SUPPORT_EMAIL} →</a><small>Support is currently provided by email.</small></div></div>
		...
		pathname,
		pageClass: "policy-shell"
	});
}
//#endregion
//#region src/pages/free-snapshot.js
function freeSnapshotPage() {
...
//#endregion
//#region src/pages/project-intake.js
var eligiblePlans = /* @__PURE__ */ new Set(["complete_revamp", "cinematic_scroll"]);
var inspirationSites = [
	[
		"Editorial commerce",
		"Aesop",
		"https://www.aesop.com/",
		"Quiet typography, generous pacing, and product storytelling."
	],
	[
		"Clear technology",
		"Stripe",
		"https://stripe.com/",
		"Strong hierarchy, technical clarity, and confident interaction."
	],
	[
		"Story-led retail",
		"Patagonia",
		"https://www.patagonia.com/",
		"Mission, documentary imagery, and useful commerce structure."
	],
	[
		"Cinematic product",
		"Apple AirPods Pro",
		"https://www.apple.com/airpods-pro/",
		"Scroll pacing, product focus, and controlled motion."
	],
	[
		"Bold campaign",
		"Nike",
		"https://www.nike.com/",
		"Large imagery, direct messaging, and fast category paths."
	]
];
function readIntakePlan(search = globalThis.location?.search || "") {
...
var pageChoice = (value, label, description) => `<label class="page-choice"><input type="checkbox" name="pages" value="${value}"><span><strong>${label}</strong><small>${description}</small></span></label>`;
var referenceCard = ([type, name, url, description]) => `<label class="style-reference" data-style-reference><input type="checkbox" name="inspirationChoice" value="${escapeHtml(url)}"><span class="micro-label">${type}</span><strong>${name}</strong><p>${description}</p><a href="${url}" target="_blank" rel="noopener noreferrer">Visit website <span aria-hidden="true">↗</span></a></label>`;
...
function operatorPage() {
	return shell(`<section class="dashboard-section"><div class="container-wide"><span class="eyebrow">Restricted operations</span><h1>Evidence and delivery control room</h1><div class="notice"><strong>Sending kill switch:</strong> <span data-sending-state>checking</span>. This product contains queue controls but no sending provider.</div><div class="notice"><strong>Official preparation ramp:</strong> days 1–4 target 15 reviewed drafts/day; days 5–14 target 20; day 15 onward requested target 22. The enforced ceiling remains 20 and sending remains disabled. This is daily capacity for new eligible businesses, never repeated contact forever.</div><div data-operator-state="loading" data-operator-content role="region" aria-live="polite"><p>Verifying operator access…</p></div></div></section>`, { pathname: "/operator" });
}
//#endregion
//#region src/pages/results.js
function resultPage(success) {
	if (success) return shell(`<section class="result-section"><div class="result-card" data-checkout-result><span class="eyebrow">Payment verification</span><h1 data-checkout-result-heading>Verifying your payment…</h1><p data-checkout-result-message role="status" aria-live="polite">The browser redirect is not payment proof. AccessRevamp is checking the durable order created by the signed Stripe webhook.</p><div class="hero-actions"><a class="button" href="/account/projects" data-nav data-checkout-result-action>Open the workspace ${icon("arrow")}</a><a class="button button--ghost" href="/contact" data-nav>Contact us</a></div></div></section>`, { pathname: "/success" });
...
//#endregion
//#region src/pages/under-construction.js
...
function underConstructionPage$1() {
	return buildUnderConstructionPage();
}
//#endregion
//#region src/cinematic-scroll.js
...
//#endregion
//#region src/services/contact.js
function setupContactForm() {
	...
	const onSubmit = async (event) => {
		...
		try {
			...
			status.textContent = error.message === "Failed to fetch" ? "The preview is not connected to the contact backend. Your message was not sent." : error.message;
		} finally {
...
var STORAGE_KEY$1 = "supabase.auth.token";
...
function getSupabase() {
	...
	if (!client) client = createClient(siteConfig.supabaseUrl, siteConfig.supabaseKey, { auth: {
		persistSession: true,
		autoRefreshToken: true,
		detectSessionInUrl: true
	} });
...
var SIGNUP_START_ENDPOINT = "/api/auth-signup-start";
var SIGNUP_RESEND_ENDPOINT = "/api/auth-signup-resend";
...
function saveLoginHint(email) {
	try {
		sessionStorage.setItem(LOGIN_HINT_KEY, email);
	} catch {}
}
...
async function waitForSession(supabase, timeoutMs = 9e3) {
	...
	return new Promise((resolve, reject) => {
		...
		timeout = setTimeout(() => finish(reject, /* @__PURE__ */ new Error("The verification email did not create a valid session.")), timeoutMs);
	});
}
function setupAuthForm(navigate) {
	const page = document.querySelector("[data-auth-page]");
	const form = document.querySelector("[data-auth-form]");
	if (!page || !form) return void 0;
	...
	const startSignup = async (fullName, email, password) => {
		...
		if (response.status === 409 && result.code === "ACCOUNT_EXISTS") return {
			accountExists: true,
...
//#endregion
//#region src/services/account-projects.js
var money = (cents = 0, currency = "USD") => new Intl.NumberFormat("en-US", {
	style: "currency",
	currency: String(currency || "USD").toUpperCase()
}).format(cents / 100);
var show = (host, name, html) => {
	host.dataset.accountState = name;
	host.innerHTML = html;
};
var label$1 = (value = "") => String(value).replace(/[_-]+/g, " ").replace(/\b\w/g, (character) => character.toUpperCase());
var date = (value, fallback = "Not scheduled") => {
	if (!value) return fallback;
	const parsed = new Date(value);
	return Number.isFinite(parsed.getTime()) ? new Intl.DateTimeFormat("en-US", { dateStyle: "medium" }).format(parsed) : fallback;
};
var dateTime = (value, fallback = "") => {
	if (!value) return fallback;
	const parsed = new Date(value);
...
var renderStatus = (value, extra = "") => `<span class="status-pill ${extra}">${escapeHtml(label$1(value || "pending"))}</span>`;
function renderUpdates(updates = []) {
    ...
    <div class="portal-meta">${update.stage ? `<span>${escapeHtml(label$1(update.stage))}</span>` : ""}${Number.isInteger(update.progress_percent) ? `<span>${update.progress_percent}% complete</span>` : ""}</div></div>
  </li>`).join("")}</ol>`;
}
function renderWorkflow(workflow) {
	if (!workflow) return "<p class=\"portal-empty\">The workflow will appear after the project is scheduled.</p>";
	const tasks = workflow.tasks || [];
	return `<div class="portal-workflow-summary"><div><span>Current stage</span><strong>${escapeHtml(label$1(workflow.current_stage))}</strong></div><div><span>Workflow</span><strong>${escapeHtml(label$1(workflow.status))}</strong></div><div><span>Revision round</span><strong>${Number(workflow.revision_round || 0)} of 2</strong></div></div>
    ${tasks.length ? `<ol class="portal-task-list">${tasks.map((task) => `<li data-task-status="${escapeHtml(task.status)}"><span>${String(task.sequence_number).padStart(2, "0")}</span><div><strong>${escapeHtml(label$1(task.task_key))}</strong><small>${escapeHtml(label$1(task.stage))}</small></div>${renderStatus(task.status)}</li>`).join("")}</ol>` : "<p class=\"portal-empty\">Detailed steps have not been published yet.</p>"}`;
}
function renderBrief(project) {
	const brief = project.brief;
	const canSubmit = ["complete_revamp", "cinematic_scroll"].includes(project.plan_key);
	if (!brief) return `<div class="portal-empty portal-empty--action"><p>${canSubmit ? "Your project brief is still needed before production can begin." : "This service does not require the full website brief."}</p>${canSubmit ? `<a class="button button--small" href="/project-intake?plan=${encodeURIComponent(project.plan_key)}&project=${encodeURIComponent(project.id)}" data-nav>Complete project brief</a>` : ""}</div>`;
    ...
    <div><span class="micro-label">Requested pages</span><p>${(brief.selected_pages || []).length ? brief.selected_pages.map((page) => escapeHtml(label$1(page))).join(" · ") : "Not specified"}</p></div>
...
function renderDesignCards(options = []) {
	...
	return `<div class="portal-design-grid">${options.map((option) => {
		...
		return `<article class="portal-design-card"><div class="portal-design-card__preview">${url ? `<a href="${escapeHtml(url)}" target="_blank" rel="noopener"><img src="${escapeHtml(url)}" alt="${escapeHtml(`${label$1(option.option_group)} option ${option.option_number}`)}" loading="lazy"/></a>` : "<div class=\"portal-file-icon\">Preview pending</div>"}</div><div><span class="micro-label">${escapeHtml(label$1(option.option_group))}</span><h4>Option ${Number(option.option_number)}</h4><p>Revision ${Number(option.revision_round || 0)}${option.scene_number ? ` · Scene ${Number(option.scene_number)}` : ""}</p>${renderStatus(option.status)}</div></article>`;
	}).join("")}</div>`;
}
var renderChoiceOptions = (options) => `<option value="">No selection</option>${options.map((option) => `<option value="${escapeHtml(option.id)}">${escapeHtml(`${label$1(option.option_group)} option ${option.option_number}`)}</option>`).join("")}`;
...
function renderWorkspace(result) {
	...
	return `${result.partialFailures?.length ? `<div class="notice"><strong>Some workspace sections are temporarily unavailable:</strong> ${result.partialFailures.map(escapeHtml).join(", ")}.</div>` : ""}<div class="customer-hub-summary"><section><span class="micro-label">Current entitlement</span><strong>${result.entitlement ? `${escapeHtml(label$1(result.entitlement.highest_tier_key))} · ${escapeHtml(label$1(result.entitlement.status))} · ${money(result.entitlement.effective_paid_cents)}` : "No paid entitlement is linked yet."}</strong></section><section><span class="micro-label">Projects</span><strong>${projects.length}</strong></section><section><span class="micro-label">Secure download links</span><strong>Refresh every ${Math.round((result.signedUrlExpiresIn || 900) / 60)} minutes</strong></section><button class="button button--ghost button--small" type="button" data-hub-refresh>Refresh workspace</button></div>
    ...
    <details class="portal-account-records"><summary>Orders and account records</summary><div class="dashboard-grid"><section class="dashboard-card"><h2>Verified orders</h2>${orders.length ? `<ul>${orders.map((order) => `<li>${escapeHtml(label$1(order.plan_key))} — ${money(order.amount_total, order.currency)} — ${escapeHtml(label$1(order.status))}</li>`).join("")}</ul>` : "<p>No verified order yet.</p>"}</section><section class="dashboard-card"><h2>Refund requests</h2>${refunds.length ? `<ul>${refunds.map((refund) => `<li>${escapeHtml(refund.reason || "Request")} — ${escapeHtml(label$1(refund.status))}</li>`).join("")}</ul>` : "<p>No refund request is on file.</p>"}</section></div></details>`;
}
function setupAccountProjects(navigate) {
	...
	const load = async () => {
		...
		try {
			const response = await fetch("/api/account-projects", { headers: { authorization: `Bearer ${session.access_token}` } });
...
//#endregion
//#region src/services/checkout.js
var CHECKOUT_ENDPOINT = "/api/create-checkout";
var ORDER_DRAFT_ENDPOINT = "/api/order-draft";
var PENDING_PLAN_KEY = "accessrevamp:pending-plan";
var ALLOWED_STRIPE_HOSTS = /* @__PURE__ */ new Set(["checkout.stripe.com"]);
var PAID_PLANS = /* @__PURE__ */ new Set([
	"homepage_reveal",
	"complete_revamp",
	"cinematic_scroll"
]);
function validatedStripeUrl(value) {
	...
	if (url.protocol !== "https:" || !ALLOWED_STRIPE_HOSTS.has(url.hostname) || url.username || url.password) throw new Error("Checkout returned an unexpected destination.");
	return url.toString();
}
...
function setupCheckout() {
	...
	const onClick = async (event) => {
		...
		if (!form) {
			try {
				sessionStorage.setItem(PENDING_PLAN_KEY, targetTier);
			} catch {}
		...
		try {
			...
			if (!draftResponse.ok || !draftPayload.draftId || !validRequestId(draftPayload.requestId)) throw new Error(draftResponse.status === 401 || draftResponse.status === 403 ? "Sign in with the confirmed project email" : draftResponse.status === 409 ? draftPayload.error || "This saved request cannot start another payment" : "Your project request was not saved — no payment started");
			...
			if (!response.ok || Object.keys(payload).length !== 1 || !payload.url) throw new Error(response.status === 503 ? "Secure checkout is paused — your request is saved" : response.status === 409 ? "The previous Checkout attempt ended — click once more to safely start a fresh attempt" : "Checkout is temporarily unavailable — your request is saved");
			...
			if (document.contains(control) && ![
				"Sign in to continue",
				"Complete the project request",
				"Reload and try again"
			].includes(control.textContent) && !control.textContent.includes("saved") && !control.textContent.includes("unavailable") && !control.textContent.includes("ended")) control.innerHTML = originalHtml;
...
//#endregion
//#region src/services/checkout-result.js
var SESSION_PATTERN = /^cs_(test|live)_[A-Za-z0-9_]+$/;
var STORAGE_KEY = "accessrevamp-order-draft-v1";
var RETRY_DELAYS_MS = [
	0,
	1200,
	2500,
...
function setupCheckoutResult(root = document) {
...
//#endregion
//#region src/config/analytics-events.js
var ANALYTICS_EVENTS = /* @__PURE__ */ new Set([
	"free_snapshot_started",
	"preview_viewed",
	"plan_viewed",
	"plan_selected",
	"checkout_started",
	...
	"water_loss_checked",
	"eta_checked",
	"private_pricing_opened"
]);
var ANALYTICS_PROPERTY_KEYS = /* @__PURE__ */ new Set([
	"route",
	"tier",
	"status",
	"demo",
...
//#endregion
//#region src/services/analytics.js
var BLOCKED_KEY = /email|token|secret|password|stripe|url|address|accessnotes|message/i;
var consent = false;
var transport = null;
function sanitizeProperties(properties = {}) {
	...
	for (const [key, value] of Object.entries(properties)) {
		...
		if (![
			...
			"number",
			"boolean"
		].includes(typeof value)) throw new Error(`Analytics property must be scalar: ${key}`);
...
//#endregion
//#region src/services/free-snapshot.js
var REQUEST_KEY = "accessrevamp:snapshot-request-id";
var requestId = () => {
...
function setupFreeSnapshot() {
	...
	const onSubmit = async (event) => {
		...
		try {
			...
			status.textContent = `${error.message} Your information is still in the form; please try again.`;
		} finally {
...
//#endregion
//#region src/services/project-intake.js
var MAX_FILES = 8;
var MAX_BYTES = 8 * 1024 * 1024;
var acceptedTypes = /* @__PURE__ */ new Set([
	"image/jpeg",
	"image/png",
	"image/webp",
	"image/avif"
]);
function setupProjectIntake() {
	...
	const onSubmit = async (event) => {
		event.preventDefault();
		if (!form.reportValidity()) {
			status.textContent = "Complete the highlighted fields before sending.";
			return;
		}
		const pages = new FormData(form).getAll("pages");
		if (!pages.length || pages.length > 5) {
			status.textContent = "Choose between one and five pages.";
			return;
		}
		const supabase = getSupabase();
		if (!supabase) {
			status.textContent = "Customer sign-in is not configured in this deployment. Your brief was not sent.";
			return;
		}
		const session = (await supabase.auth.getSession()).data?.session;
		if (!session) {
			status.textContent = "Sign in with the confirmed checkout email before sending this brief.";
			return;
		}
...
var API_ENDPOINT = "/api/operator-overview";
var DEFAULT_MAXIMUM_BYTES = 50 * 1024 * 1024;
var CUSTOMER_HUB_URL = "https://accessrevamp.com/account/projects";
var label = (value = "") => String(value).replace(/[_-]+/g, " ").replace(/\b\w/g, (character) => character.toUpperCase());
...
function renderUpdateForm(projects) {
    ...
    <div class="operator-form__split operator-form__split--three"><label>Progress %<input name="progressPercent" type="number" min="0" max="100" step="1" placeholder="45" /></label><label>Project status<select name="projectStatus"><option value="">No change</option>${[
		"intake_pending",
		"reviewing",
		"concept",
		"implementation",
		...
		"paused",
		"canceled"
	].map((value) => `<option value="${value}">${label(value)}</option>`).join("")}</select></label><label>Delivery status<select name="deliveryStatus"><option value="">No change</option>${[
		"waiting_for_inputs",
		"scheduled",
		"in_progress",
		"ready_for_delivery",
		...
		"paused",
		"canceled"
	].map((value) => `<option value="${value}">${label(value)}</option>`).join("")}</select></label></div>
...
function renderArtifactForm(projects, uploadLimits = {}) {
    ...
    <div class="operator-form__head"><div><span class="micro-label">Private deliverables</span><h3>Upload a file to a customer</h3></div><span class="status-pill">Up to ${escapeHtml(formatBytes(Number(uploadLimits.maximumBytes || DEFAULT_MAXIMUM_BYTES)))}</span></div>
    <label>Project<select name="projectId" required>${projectOptions(projects)}</select></label>
...
function renderOverview(data) {
	...
	return `${data.partialFailures?.length ? `<div class="notice"><strong>Partial operator data:</strong> ${data.partialFailures.map(escapeHtml).join(", ")}.</div>` : ""}<div class="operator-hub-actions"><button class="button button--ghost button--small" type="button" data-operator-refresh>Refresh control room</button></div>
...
function setupOperator() {
	...
	const api = async (payload) => {
		...
		return readResponse(await fetch(API_ENDPOINT, {
	...
	const load = async (flash = "", emailLink = "") => {
		...
		try {
			...
			const flashMarkup = flash ? `<div class="notice notice--success"><strong>${escapeHtml(flash)}</strong>${emailLink ? `<p><a class="button button--small" href="${escapeHtml(emailLink)}">Open customer email</a></p>` : ""}</div>` : "";
	...
	const submitArtifact = async (form) => {
		...
		try {
			...
			const upload = await supabase.storage.from(slot.bucket).uploadToSignedUrl(slot.path, slot.token, file, {
				contentType,
				cacheControl: "3600"
			});
			...
			const emailLink = values.get("prepareEmail") === "on" ? deliveryEmail(project, title, artifactType) : "";
			await load(values.get("markDelivered") === "on" ? "The final website package is available to the customer." : "The file is available in the customer hub.", emailLink);
		} catch (error) {
			if (slot?.artifactId && /upload/i.test(status.textContent)) await api({
				action: "cancel_artifact_upload",
				artifactId: slot.artifactId
			}).catch(() => void 0);
...
//#endregion
//#region src/services/pricing-context.js
...
function readPrivatePricingToken(locationValue = location) {
...
function normalizeContext(value) {
	...
	if (website.protocol !== "https:" || !label || scope.length < 20 || Number.isNaN(expiry.getTime()) || expiry <= /* @__PURE__ */ new Date()) throw new Error("Invalid private context.");
	return {
		label,
		scope,
		website,
		expiry,
		tier: value.recommended_tier
	};
}
function setupPricingContext(root = document) {
	...
	fetch("/api/pricing-context", {
		...
		host.querySelector("[data-private-expiry]").textContent = context.expiry.toLocaleDateString(void 0, { dateStyle: "medium" });
...
var routes = {
	...
	"/accessibility": () => legalPage("accessibility", "/accessibility"),
	"/refunds": () => legalPage("refunds", "/refunds"),
	"/legal": () => legalPage("legal", "/legal"),
	"/outreach-standards": () => legalPage("outreach", "/outreach-standards"),
	"/support": () => legalPage("support", "/support"),
	"/customer-support": () => legalPage("support", "/customer-support"),
...
function setupMenu() {
	...
	const setOpen = (next, { restoreFocus = false, focusMenu = false } = {}) => {
		...
		if (open && focusMenu) requestAnimationFrame(() => menu.querySelector("a")?.focus({ preventScroll: true }));
		else if (!open && restoreFocus) button.focus({ preventScroll: true });
	};
...
function renderRoute({ pathname, pattern, params, view }) {
	...
	if (pattern === "/portfolio/:slug") {
		...
		if (!load) app.innerHTML = notFoundPage();
		else load().then(([pageModule, setupModule]) => {
			if (!active) return;
...
var cleanupCheckout = setupCheckout();
