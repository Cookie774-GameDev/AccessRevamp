...
function updateDocumentMetadata(pathname, pattern = pathname) {
	...
	document.querySelector("meta[name=\"robots\"]")?.setAttribute("content", pattern === "/preview/:token" ? "noindex,nofollow" : "index,follow,max-image-preview:large");
}
//#endregion
...
//#endregion
//#region src/data/lenses.js
var lenses = [
	{
		title: "Accessibility",
		summary: "Can everyone perceive, navigate, and complete the task?",
		visual: "focus-path",
		checks: [
			"Keyboard path",
			"Contrast and labels",
		...
		outcome: "A prioritized barrier-removal brief.",
		tone: "mint"
	},
	{
		title: "Usability",
		summary: "Does the next useful step feel obvious?",
		visual: "task-flow",
		checks: [
		...
		outcome: "A clearer route through the main task.",
		tone: "sun"
	},
	{
		title: "Mobile",
		summary: "Does the hierarchy survive a narrow screen?",
		visual: "mobile-frame",
		checks: [
			"Reading order",
			"Tap targets",
		...
		outcome: "A mobile direction with the same intent.",
		tone: "coral"
	},
	{
		title: "Performance",
		summary: "Does the page make visitors wait for meaning?",
		...
		outcome: "A faster path to visible usefulness.",
		tone: "ink"
	},
	{
		title: "Content",
		summary: "Can someone understand the offer quickly?",
		...
		outcome: "Copy with a deliberate information order.",
		tone: "cream"
	},
	{
		title: "SEO / local discovery",
		summary: "Can the right visitor find the right page?",
		...
		outcome: "A more legible discovery structure.",
		tone: "mint"
	},
	{
		title: "Conversion",
		summary: "Does one primary action lead the decision?",
		visual: "cta-path",
		checks: [
			"Action weight",
			"Objection coverage",
		...
		outcome: "A decision path with less competition.",
		tone: "coral"
	},
	{
		title: "Monetization",
		summary: "Are price, scope, and upgrades understandable?",
		...
		outcome: "A commercial offer people can compare.",
		tone: "sun"
	},
	{
		title: "Analytics",
		summary: "Can useful decisions be measured safely?",
		visual: "event-funnel",
		checks: [
			"Task starts",
			"Completion signals",
		...
		outcome: "A small measurement plan tied to decisions.",
		tone: "ink"
	},
	{
		title: "Social growth",
		summary: "Does the website create material worth sharing?",
		visual: "publishing-rhythm",
		checks: [
			"Proof moments",
			"Teaching material",
			"Destination pages"
		],
		outcome: "A useful publishing rhythm grounded in the site.",
		tone: "cream"
	},
	{
		title: "Security hygiene",
		summary: "Are public trust and testing boundaries explicit?",
		visual: "secure-form",
		checks: [
			"Trust cues",
			"Form privacy",
		...
		outcome: "Clearer trust language and safer next steps.",
		tone: "mint"
	}
].map((lens, index) => ({
	...lens,
...
//#endregion
//#region src/components/lens-visuals.js
var bars = (values) => values.map((value) => `<i style="--v:${value}%"></i>`).join("");
function lensVisual(key) {
	return {
		...
		"load-timeline": `<span class="lens-ui lens-ui--bars">${bars([
			32,
			56,
			78,
			94
		])}</span>`,
		"content-stack": `<span class="lens-ui lens-ui--copy"><b></b>${bars([
			92,
			76,
			54
		])}</span>`,
		...
		"event-funnel": `<span class="lens-ui lens-ui--funnel">${bars([
			96,
			72,
			48,
			28
...
var demos$1 = [
	{
		href: "/portfolio/verdant-cut",
		number: "01",
		name: "Verdant Cut Co.",
		type: "Local service editorial system",
		action: "Build a service plan",
		art: "greenline",
		asset: demoBrands.greenline.interface
	},
	{
		href: "/portfolio/ember-and-jar",
		number: "02",
		name: "Ember & Jar",
		type: "Tactile food commerce",
		action: "Choose a heat level",
		art: "firejar",
		asset: demoBrands.firejar.interface
	},
	{
		href: "/portfolio/clearline-plumbing",
		number: "03",
		name: "Clearline Plumbing",
		type: "Technical local-service journey",
		action: "Choose the right repair path",
		art: "clearflow",
		asset: demoBrands.clearflow.interface
	}
];
var outcomes = [
	[
...
var demoCard$1 = (demo) => `<article class="demo-card demo-card--${demo.art}" data-reveal>
  <a class="demo-card__visual" href="${demo.href}" data-nav aria-label="Open ${demo.name} portfolio concept">
    <span class="demo-card__image">${picture(demo.asset, {
	alt: `${demo.name} responsive fictional website concept`,
...
var STORAGE_KEY$1 = "accessrevamp-order-draft-v1";
var MAX_FILES$1 = 8;
...
function setupHomeExperience(root = document) {
	...
	hero?.querySelector("[data-reveal-cursor]");
	...
	if (hero) {
		...
		listen(hero, "pointerleave", () => {
			...
			navTimer = setTimeout(() => {
				if (scrollY < 24 && !shell?.querySelector(".site-header:focus-within")) shell?.classList.remove("nav-is-visible");
			}, 520);
		});
	...
	tiles.forEach((tile) => {
		...
		listen(tile, "focus", () => {
			if (keyboardMode || finePointer?.matches) setActive(tile);
		});
		listen(tile, "keydown", (event) => {
			...
			if (event.key === "Enter" || event.key === " ") {
				event.preventDefault();
				setActive(active === tile ? null : tile);
			}
		});
	});
	listen(grid, "pointerleave", () => {
		clearTimeout(intentTimer);
		if (finePointer?.matches) setActive(null);
	});
...
var demos = [
	{
		brand: demoBrands.greenline,
		task: "Eligibility, quote, compare, and sample scheduling",
		tone: "greenline"
	},
	{
		brand: demoBrands.firejar,
		task: "Heat filtering, local demo cart, and safe checkout states",
		tone: "firejar"
	},
	{
		brand: demoBrands.clearflow,
		task: "Urgent/planned paths, sample ETA, and water-loss illustration",
		tone: "clearflow"
	}
];
var demoCard = ({ brand, task, tone }, index) => `<article class="demo-card demo-card--${tone}">
  <a class="demo-card__visual" href="/portfolio/${brand.slug}" data-nav aria-label="Open ${brand.name} working demonstration">
    <span class="demo-card__image">${picture(brand.interface, {
	alt: `${brand.name} working demonstration shown in a desktop browser`,
...
function setupCheckout() {
	...
	const onClick = async (event) => {
		...
		try {
			...
			if (!response.ok || Object.keys(payload).length !== 1 || !payload.url) throw new Error("Checkout is temporarily unavailable.");
			location.assign(validatedStripeUrl(payload.url));
		} catch {
			setCheckoutFailure(control, "Checkout unavailable — try again");
			return;
		} finally {
			if (document.contains(control) && !["Sign in to continue", "Checkout unavailable — try again"].includes(control.textContent)) control.innerHTML = originalHtml;
