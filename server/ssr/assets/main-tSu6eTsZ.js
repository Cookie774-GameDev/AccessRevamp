...
function shell(content, { pathname = location.pathname, home = false, pageClass = "" } = {}) {
    ...
    <footer class="site-footer">
      ...
      <div class="container-wide footer-bottom"><span>© ${(/* @__PURE__ */ new Date()).getFullYear()} AccessRevamp</span><span>One-time services · Clear scope · No outreach without human approval</span></div>
    </footer>
  </div>`;
}
//#endregion
...
var pages = {
	...
	accessibility: {
		...
		items: [
			"The interface uses semantic structure, keyboard navigation, visible focus, readable contrast, reduced-motion support, and announced form status.",
			"Automated checks are paired with manual review; neither alone proves full conformance.",
			"When reporting a barrier, include the page, task, browser, and assistive technology if comfortable.",
			"Confirmed issues are documented with a resolution or known limitation."
		]
	},
	legal: {
...
function setupAuthForm(navigate) {
	...
	const onSubmit = async (event) => {
		...
		try {
			const result = form.dataset.mode === "signup" ? await supabase.auth.signUp({
				email,
				password,
			...
			if (form.dataset.mode === "signup" && !result.data.session) status.textContent = "Check your email to confirm the account, then sign in.";
			else navigate("/account/projects");
		} catch (error) {
...
//#endregion
//#region src/services/dashboard.js
var empty = (title, message, action = "") => `<div class="empty-state"><h2>${title}</h2><p>${message}</p>${action}</div>`;
...
function setupOperator() {
	...
	const submitArtifact = async (form) => {
		...
		try {
			...
			await load(values.get("markDelivered") === "on" ? "The final website package is available to the customer." : "The file is available in the customer hub.");
		} catch (error) {
