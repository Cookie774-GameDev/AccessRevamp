...
function setupCheckout() {
	...
	const onClick = async (event) => {
		...
		try {
			...
			if (!draftResponse.ok || !draftPayload.draftId) throw new Error(draftResponse.status === 401 || draftResponse.status === 403 ? "Sign in with the confirmed project email" : "Your project request was not saved — no payment started");
			...
			if (!response.ok || Object.keys(payload).length !== 1 || !payload.url) throw new Error(response.status === 503 ? "Secure checkout is paused — your request is saved" : "Checkout is temporarily unavailable — your request is saved");
			...
			].includes(control.textContent) && !control.textContent.includes("saved") && !control.textContent.includes("unavailable")) control.innerHTML = originalHtml;
