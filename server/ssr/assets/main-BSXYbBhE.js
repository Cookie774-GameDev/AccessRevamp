...
var MEDIA_SYNC_EPSILON_SECONDS = 1 / 30;
var MAX_SEEK_STEP_SECONDS = 1 / 12;
var FORWARD_PLAY_THRESHOLD_SECONDS = .18;
var FORWARD_CATCHUP_WINDOW_SECONDS = .32;
var MIN_PLAYBACK_RATE = .75;
var MAX_PLAYBACK_RATE = 2;
var FRAME_SETTLE_TIMEOUT_MS = 120;
...
function setupShowcaseComparisons(root = document) {
	...
	const scheduleVideoDrive = (video, state) => {
		...
		if (!video.paused && video.requestVideoFrameCallback) state.frameCallbackId = video.requestVideoFrameCallback(run);
		else state.animationFrameId = requestAnimationFrame(run);
	};
	...
	const seekOneStep = (video, state, difference) => {
		if (state.pendingSeek) return;
		if (!video.paused) video.pause();
		const step = clamp(difference, -.08333333333333333, MAX_SEEK_STEP_SECONDS);
		const nextTime = clamp(video.currentTime + step, 0, Math.max(0, video.duration - .001));
		if (Math.abs(nextTime - video.currentTime) < .001) return;
	...
	const driveVideo = (video, state) => {
		...
		if (difference > FORWARD_PLAY_THRESHOLD_SECONDS && !reducedMotion) {
			...
			if (Math.abs(video.playbackRate - nextRate) > .05) video.playbackRate = nextRate;
			if (video.paused && !state.playPromise) {
				...
				if (playPromise?.then) {
					...
					playPromise.then(() => {
						...
						if (state.playPromise === playPromise) state.playPromise = null;
						if (!destroyed && pageVisible && state.chapterIndex === activeIndex) seekOneStep(video, state, state.targetTime - video.currentTime);
					});
					return;
				}
			}
	...
	const animateTowardScroll = (time) => {
		...
		const maxStep = MAX_PROGRESS_PER_SECOND * (delta / 1e3);
	...
	const ensureSmoothing = () => {
		if (!smoothingFrame && !reducedMotion && pageVisible && activeIndex >= 0) smoothingFrame = requestAnimationFrame(animateTowardScroll);
	};
...
function accountProjectsPage() {
	return shell(`<section class="dashboard-section"><div class="container-wide"><div class="dashboard-head"><div><span class="eyebrow">Secure customer workspace</span><h1>Your projects and deliverables</h1><p data-account-greeting>Checking your confirmed session…</p></div><button class="button button--ghost" type="button" data-account-logout hidden>Sign out</button></div><div data-account-state="loading" data-account-content role="region" aria-live="polite"><div class="loading-card"><p>Loading verified purchases, projects, deliverables, and requests…</p></div></div><aside class="notice"><strong>Need help?</strong> Use the contact page. Never send credentials or private customer data in a support message.</aside></div></section>`, { pathname: "/account/projects" });
}
//#endregion
...
var money = (c = 0, currency = "USD") => new Intl.NumberFormat("en-US", {
	style: "currency",
...
function setupAccountProjects(navigate) {
	...
	if (!host) return;
	const logout = document.querySelector("[data-account-logout]"), greeting = document.querySelector("[data-account-greeting]");
	let disposed = false;
	const load = async () => {
		const supabase = getSupabase();
		if (!supabase) {
			show(host, "configuration-missing", "<h2>Workspace configuration missing</h2><p>This preview has no Supabase public configuration.</p>");
			return;
		}
		...
		if (sessionResult.error) {
			show(host, "session-expired", "<h2>Session unavailable</h2><p>Sign in again to continue.</p>");
			return;
		}
		if (!session) {
			show(host, "signed-out", "<h2>Sign in to continue</h2><p><a class=\"button\" href=\"/login\" data-nav>Sign in</a></p>");
			return;
		}
...
function setupOperator() {
	...
	const load = async () => {
		...
		if (!session) {
			...
			host.innerHTML = "<h2>Operator sign-in required</h2><a class=\"button\" href=\"/login\" data-nav>Sign in</a>";
			return;
		}
		try {
			const response = await fetch("/api/operator-overview", { headers: { authorization: `Bearer ${session.access_token}` } }), data = await response.json();
			if (!response.ok) throw new Error(data.error || "Access denied");
			if (disposed) return;
			document.querySelector("[data-sending-state]").textContent = data.sendingEnabled ? "unexpectedly enabled — investigate" : "disabled";
