...
export const orderDraftTextSchema = z.object({
  ...
  mainGoal: z.string().trim().min(2).max(4000),
  requestedPages: z.string().trim().min(2).max(4000),
  integrations: z.string().trim().min(2).max(4000),
  styleDirection: z.string().trim().max(4000).default(''),
  contentStatus: z.string().trim().max(120).default(''),
